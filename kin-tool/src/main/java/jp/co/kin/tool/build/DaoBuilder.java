...
public class DaoBuilder extends SourceBuilder {
            ...
            for (String table : this.targetTableList) {
                ...
                /* Dao生成 */
                {
                    ...
                    for (Row row : excel.getRowList()) {
                        ...
                        Class<?> entityClass = Class
                                .forName("jp.co.kin.db.entity."
                                        + toJavaFileName(getPhysicalName(row)));
                    ...
                    fileConf.setFileName(toJavaFileName(table) + "Dao"
                ...
                /* DaoTest生成 */
                {
                    ...
                    Class<?> daoClass = Class
                            .forName("jp.co.kin.db.dao." + toJavaFileName(table) + "Dao");
                    Field serviceField = new Field("dao", "dao", daoClass,
                            AccessType.PRIVATE,
                            annotationMap);
                    ...
                    fileConf.setOutputPath(this.baseDir
                            + "\\kin-db\\src\\test\\java\\jp\\co\\kin\\db\\dao");
                    fileConf.setFileName(toJavaFileName(table) + "DaoTest"
                            + FileExtension.JAVA.getValue());
    ...
    private Method getDeleteMethod(Field field, JavaSource source) {
        ...
        Method deleteMethod = new Method(field, AccessType.PUBLIC) {
            ...
            public String toString() {
                ...
                body.add(
                        TAB + this.accessType.getValue() + " int " + getMethodName() + "("
                                + signature.toString() + ");");
    ...
    private String build(JavaSource source) {
        ...
        // class情報
        result.add(buildClassJavaDoc(source)
                + buildClassAnnotation(source.getClassAnnotationMap())
                + LineFeedType.CRLF.getValue() + buildClass(source)
                + buildInterfaces(source) + " {");
